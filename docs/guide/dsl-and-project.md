...
- 当前主要通过文档和 OpenSpec 维护 DSL 配置面，不额外维护容易过期的外部示例
...
- `seeds[].request` / `follow[].request` 继续走同一个 `Request` 模型
...
- 去重、并发、间隔、限速、自适应节流、重试等能力属于框架本身的 engine 能力；在 DSL v1 里会统一映射到 `engine.dedup / engine.concurrency / engine.interval / engine.rate_limit / engine.auto_throttle / engine.retry_by_status / engine.retry_by_error`
- `output.sinks` 会继续走统一的 store 路由
