...
- 当前 `robots.txt` 已支持默认关闭、按 origin 缓存、`User-agent` / `Allow` / `Disallow`、`Crawl-delay`、`Request-rate`、更完整 `group` 选择，以及 `* / $` wildcard 规则；现在也按统一 URL 语义解析规则目标，能正确处理常见的 UTF-8 BOM、absolute URL 和 protocol-relative 规则值；其中 `Request-rate` 当前按均匀间隔最小 delay 生效；默认 cache backend 仍是内存，也已补内置 `robots::cache::File`、`cache_ttl` 刷新、临时不可用时的可配置 fail-open / fail-closed 策略、临时不可用时的 retry delay 退避、per-origin `with_site_policy(...)` 站点策略 overlay，以及可选 sitemap 自动种子；当前仍未补的是更高阶站点策略
