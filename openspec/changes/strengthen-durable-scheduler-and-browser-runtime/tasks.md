...
- [ ] 1.4 在 `src/request/browser.rs` 定义结构化 custom fingerprint profile 与显式 session reuse 策略，并保持旧的内置 profile API 可用。
- [ ] 1.5 调整 `src/download/browser.rs` 的执行计划与 session 路径，让内置 preset、自定义 profile 和 reuse 策略走同一条运行时逻辑。
- [ ] 1.6 补 browser 高级能力示例，并同步 `README.md`、`docs/capabilities.md`。
- [ ] 1.7 运行 `cargo test --quiet` 验证整轮变更。
