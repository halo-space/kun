...
- 当前 `robots.txt` 已支持默认关闭、按 origin 缓存、`User-agent` / `Allow` / `Disallow`、`Crawl-delay`、`Request-rate`、更完整 `group` 选择，以及 `* / $` wildcard 规则；其中 `Request-rate` 当前按均匀间隔最小 delay 生效；默认 cache backend 仍是内存，也已补内置 `robots::cache::File`、`cache_ttl` 刷新、临时不可用时的可配置 fail-open / fail-closed 策略，以及可选 sitemap 自动种子；当前仍未补的是更高阶语法细节与更复杂站点策略
